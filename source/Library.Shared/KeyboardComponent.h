...
namespace Library
{
	...
	private:
		static unique_ptr<DirectX::Keyboard> sKeyboard;
	...
	template <typename T>
	void UpdateValueWithKeyboard(const KeyboardComponent& keyboard, const Keys increaseKey, const Keys decreaseKey, T& value, const T& delta, function<void(const T&)> updateFunc = nullptr, const T& minValue = numeric_limits<T>::lowest(), const T& maxValue = { numeric_limits<T>::max() })
	{
	...
	template <typename T>
	void IncrementEnumValue(const KeyboardComponent& keyboard, const Keys increaseKey, T& value, function<void(T)> updateFunc, T maxValue, T minValue = T(0), bool wrap = true)
	{
